...
TEST_F(SHMServiceTest, BasicServiceCallTest)
{
    ...
    std::thread server_thread([&]() {
        irlab::shm::ServiceServer<int, int> server("/test_service", addOneService);
...
TEST_F(SHMServiceTest, NonPODClassErrorTest)
{
    ...
    try {
        irlab::shm::ServiceServer<NonPODClass, int> server("/test_service", nonPODClassService);
    ...
    try {
        irlab::shm::ServiceClient<NonPODClass, int> client("/test_service");
        FAIL() << "Expected std::runtime_error";
    } catch (const std::runtime_error& e) {
        EXPECT_TRUE(true);
    }
...
{
    std::thread server_thread([&]() {
        irlab::shm::ServiceServer<int, int> server("/test_service", multiplyByTwo);
...
{
    std::thread server_thread([&]() {
        irlab::shm::ServiceServer<LargeData, LargeData> server("/test_service", processLargeData);
        std::this_thread::sleep_for(std::chrono::seconds(2));
    });
    
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    
    irlab::shm::ServiceClient<LargeData, LargeData> client("/test_service");
